#!/usr/bin/env python3
# Heavy DDP scaling benchmark for ContextualizedCorrelationNetworks with cached preprocessing and compute-intensive settings (no CSV output).
...
def load_and_preprocess(
    ...
    if use_cache and os.path.exists(cache_path):
        ...
        return (
            ...
            cached["C_test"],
            cached["X_test_norm"],
            cached["cell_ids_train"],
            cached["cell_ids_test"],
        )
    ...
    def build_context(df_split, X_scaled, morgan, pt, pd, ign_t, ign_d, scaler=None, fit=False):
    ...
    X_train, C_train, cell_ids_train, ctx_scaler = build_context(
        df_train, X_train_scaled, morgan_train, pt_train, pd_train, ign_t_train, ign_d_train, fit=True
    )
    X_test, C_test, cell_ids_test, _ = build_context(
        df_test, X_test_scaled, morgan_test, pt_test, pd_test, ign_t_test, ign_d_test, scaler=ctx_scaler
    )
...
def run_ccn_benchmark(
    ...
    encoder_params = (
        context_dim * encoder_width
        + encoder_width * encoder_width * (encoder_layers - 1)
        + encoder_width * num_archetypes
    )
...
def parse_args():
    ...
    ap = argparse.ArgumentParser(description="Heavy ContextualizedCorrelationNetworks Scaling Benchmark (no CSV output)")
...
def main():
    ...
    label = args.label or f"{world_size}gpu_ccn_heavy"

    print_rank0("\n" + "=" * 70)
    print_rank0("HEAVY ContextualizedCorrelationNetworks SCALING BENCHMARK (NO CSV)")
    print_rank0("=" * 70)
    print_rank0(f"  World size: {world_size}")
